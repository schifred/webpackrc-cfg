...
let install = exports.install = (() => {
  var _ref = _asyncToGenerator(function* (name) {
    ...
    if (!_fs2.default.existsSync(modulePath)) {
      ...
      yield (0, _exec2.default)(`${_config.npm} install ${nameWithVersion}`, {
...
let installDependency = exports.installDependency = (() => {
  var _ref2 = _asyncToGenerator(function* () {
    for (let i = 0; i < peerDependencies.length; i++) {
      const [moduleName] = getDependencyNameWithVersion(peerDependencies[i]) ? peerDependencies[i] : peerDependencies[i].split(/\.|\//);
...
var _hyphen = require('./utils/hyphen');
...
function transformDependencyName(name) {
  ...
  return (0, _hyphen.hyphen)(name);
};

function getDependencyNameWithVersion(name) {
  let version = _package.projectDependencies[name];
