...
var _ntils = require('ntils');

var _ntils2 = _interopRequireDefault(_ntils);
...
let WebpackConfig = (_temp = _class = class WebpackConfig {
  ...
  set entry(entry) {
    this.options.entry = {};
    if (_ntils2.default.isObject(opts.entry) && !_ntils2.default.isArray(opts.entry)) {
      _ntils2.default.each(opts.entry, (nameExpr, fileExpr) => {
        let files = _globby2.default.sync(fileExpr);
        files.forEach(file => {
          let paths = file.split('/').reverse().map(item => _path2.default.basename(item).split('.')[0]);
      ...
      let files = _globby2.default.sync(opts.entry);
      entries = files.map(file => {
        let name = _path2.default.basename(file).split('.')[0];
        this.options.entry[name] = file;
      });
    };
  }
